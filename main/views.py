...
def new_project_application(request):
    # if this is a POST request we need to process the form data
    if request.method == 'POST':
        # create a form instance and populate it with data from the request:
        form = newProjectApplication(request.POST)
        # check whether it's valid:
        if form.is_valid():
            # process the data in form.cleaned_data as required
            subject = "New Project Application"
            sender_email = 'support@WebLime.com'
            recipients = ['edan@weblime.com']
            body = {
			'fullname': form.cleaned_data['fullname'],
			'email': form.cleaned_data['email'],
			'phone': form.cleaned_data['phone'],
    ...
    return render(request, 'new-project-application.html', {'form': form, 'title': 'New Project Application'})
